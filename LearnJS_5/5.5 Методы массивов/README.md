...
<br>
  ...
  - [ ] `**По умолчанию элементы сортируются как строки.**` - это значит, что `sort()` без переданной внутри ф-и сортировки - будет преобразовывать элементы в строки и сортировать элементы как строки `(лексикографический порядок)`

  ```javascript
    const array = [90, 56, 2, 6, 4, 7, 34];

    /*
      По умолчанию элементы сортируются как строки. (если не переданы аргументы)
      И по первому символу (лексикографический порядок)
    */
    console.log(array.sort()); // [ 2, 34,  4, 56, 6,  7, 90 ] // числа отсортированы как строки
    ...
    */
    ...
    console.log(sortedArray); // [ 2, 4, 6, 7, 34, 56, 90 ] // теперь сортировка чисел проходит корректно
  ...
  console.log(sortedArray); // [ 2, 4, 6, 7, 34, 56, 90 ] // такой же короткий вариант
  ```
  ...
  + [x] Типы данных так же будут преобразовываться к числу / строке в зависимости от переданного аргумента в `sort()` 
    ...
    ```javascript
      ...
      /*
        Массив состоит из разных типов данных
        Его сортировка будет зависеть от аргумента в методе sort()
          а) Аргумента нет = сравнение как строк
          б) Аргумент передан = сравнение как чисел (преобразование)
      */
      let array1 = [80, 10, "Вова", 4, "Петя", true, user, "Дебил", null];
      
      /*
        В этом случае передается аргумент для сравнения чисел
          1) Числа будут сравнены как числа в порядке возрастания
          2) Строки просто ставятся в конец
          3) У объекта сработает hint number => valueOf(), который вернет число.
              Объект будет равен числу 15
          4) Boolean преобразуется в число true = 1
          5) null = 0
      */
      ...
      ); // [null,true,4,10,{name: 'sasha',age: 15,toString: [Function: toString],valueOf: [Function: valueOf]},80,'Вова','Петя','Дебил']
      
      /*
        В этом случае передается аргумента нет == преобразуем в строки
          1) Числа = строки (сравнение лексиграфическое) + (цифры идут первее слов )
          2) Строки как строки (англ слова преоритетнее)
          3) У объекта сработает hint string => toString(), который вернет строку
              Обхект будет равен строке "sasha"
          4) Boolean стане строкой 
          5) null так же станет строкой
      */
      console.log(array1.sort());
      // [10,4,80,null,{name: 'sasha',age: 15,toString: [Function: toString],valueOf: [Function: valueOf]},true,'Вова','Дебил','Петя']
    ```
