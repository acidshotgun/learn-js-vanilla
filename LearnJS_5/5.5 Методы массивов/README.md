...
- [x] Доп. расширенный пример.

  + Он показывает, что `filter()` возвращает зн-е в новый массив, только когда результат равен `true`.
  + В данном примере `callback` всегда возвращает `true` и поэтому в новый массив перейдут все з-ня.
  + Был бы `false`, то отфильтрованный массив был бы пустым.

```javascript
  const array = [0, 90, 56, 2, 6, 4, 7, 34];

  const filteredArray = array.filter(() => true);
  
  console.log(filteredArray); // [0, 90, 56, 2, 6, 4, 7, 34]
```
...
<br>
  ...
  ```javascript
  // Короткий вариант (по возрастанию)
    const array = [90, 56, 2, 6, 4, 7, 34];
    ...
    // (по убыванию)
    const sortedArrayTwo = array.sort((a, b) => {
      return b - a; // зн-я меняются местами, чтобы сделать по убыванию
    });
    
    console.log(sortedArrayTwo); // [90, 56, 34, 7, 6, 4, 2] // такой же короткий вариант
  ```
  ...
  ```javascript
    const item = [
      { name: 'olyh', age: 23 },
      { name: 'ded', age: 44 },
      { name: 'chapalah', age: 31 },
      { name: 'shket', age: 18 }
    ];
    
    console.log(item.sort((a, b) => a.age - b.age))
  ```
...
- [x] Для добавления/удаления элементов:

  + `push(...items)` – добавляет элементы в конец => ***(возвращает новую длину массива)***.
  + `pop()` – удаляет элемент с конца => ***(возвращает удаленный элемент)***.
  + `shift()` – удаляет элемент с начала => ***(возвращает удаленный элемент)***.
  + `unshift(...items)` – добавляет элементы в начало => ***(возвращает новую длину массива)***.
     
  <br>

  + `splice(pos, deleteCount, ...items)` – начиная с индекса `pos` удаляет `deleteCount` элементов и вставляет `items` => ***(Возвращает массив из удалённых элементов)***.
  + `slice(start, end)` – создаёт новый массив, копируя в него элементы с индекса `start` до `end (не включая end)`. => ***(Возвращает массив скопированных элементов)***.
  + `concat(...items)` – возвращает новый массив: копирует все члены текущего массива и добавляет к нему `items`. Если какой-то из `items` является массивом, тогда берутся его элементы. => ***(Возвращает массив скопированных элементов)***.
     
  <br>
  <br>

- [x] Для поиска среди элементов:

  + `indexOf/lastIndexOf(item, pos)` – ищет `item`, начиная с позиции `pos`, и возвращает его индекс или `-1`, если ничего не найдено. => ***(Возвращает индекс элемента или -1)***.
  + `includes(value)` – возвращает `true`, если в массиве имеется элемент `value`, в противном случае `false`. => ***(Возвращает true или fasle)***
  + `find(func)` – фильтрует элементы через функцию и отдаёт первое валидное зн-я, при прохождении которых через функцию возвращается `true`. => ***(Возвращает первое найденое зн-е)***
  + `filter(func)` - фильтрует элементы через функцию и все валидные зн-я, при прохождении которых через функцию возвращается `true`. => ***(Возвращает отфильтрованный массив)***.
  + `some(func)` - возвращает `true`, если в массиве есть хотя бы один элемент удовлетворяющий условию `func`. => ***(Возвращает true или fasle)*** 
  + `findIndex` похож на `find`, но возвращает индекс вместо значения. => ***(Возвращает индекс валидного элемента)***.
     
  <br>
  <br>

- [x] Для перебора элементов:
...
- [x] Для преобразования массива:

  + `map(func)` – создаёт новый массив из результатов вызова `func` для каждого элемента. => ***(Возвращает новый массив)***.
  + `sort(func)` – сортирует массив «на месте», а потом возвращает его. => ***(Возвращает массив / изменяет оригинальый массив)***.
  + `reverse()` – «на месте» меняет порядок следования элементов на противоположный и возвращает изменённый массив. => ***(Возвращает массив измененный массив / Не меняет оригинальный)***.
  + `split/join` – преобразует строку в массив и обратно.=> ***(Возвращает массив / строку / Не изменяет ориг массив)***.
  + `reduce/reduceRight(func, initial)` – вычисляет одно значение на основе всего массива, вызывая `func` для каждого элемента и передавая промежуточный результат между вызовами. => ***(Возвращает `acc` - среднее значение (`может быть так же и объектом или массивом`)***.
...
- [x] Дополнительно:
  ...
  + `Array.isArray(arr)` проверяет, является ли arr массивом. => ***(Возвращает true/false)***.
